<template>
    <div class="slots-game">
      ...
      <div class="slot-machine">
        ...
        <div class="reels-container" :class="{ spinning: slotStore.isSpinning }">
          <div v-for="(reel, reelIndex) in slotStore.reels" 
               :key="reelIndex" 
               class="reel"
               :style="{ animationDelay: `${reelIndex * 0.2}s` }">
            <!-- Static Symbols -->
            <div v-for="(symbol, symbolIndex) in reel" 
                 :key="symbolIndex"
                 class="symbol"
                 :class="{ 'winning': isWinningSymbol(reelIndex, symbolIndex) }">
              <div class="symbol-inner">
                <span class="symbol-emoji">{{ getSymbolEmoji(symbol?.name) }}</span>
              </div>
            </div>
        ...
        <div class="paylines-overlay" 
          ...
          <svg class="paylines" preserveAspectRatio="none">
            <path v-for="(line, index) in slotStore.winningLines" 
                  :key="index"
  ...
  // Watch for wins
  watch(() => slotStore.lastWin, (newWin) => {
    if (newWin > 0) {
  ...
  const addNotification = (type: 'win' | 'bigWin' | 'megaWin', amount: number) => {
    const titles = {
      win: 'Nice Win!',
      bigWin: 'Big Win!',
      megaWin: 'Mega Win!'
  ...
  @media (min-width: 1024px) {
    ...
    .bet-input, .auto-spins {
      padding: 2rem;
      border-radius: 20px;
    }
  
    .quick-amounts, .auto-spins {
      display: grid;
  ...
  .payline-0 { stroke: #ff4081; animation-delay: 0s; }
  .payline-1 { stroke: #40c4ff; animation-delay: 0.2s; }
  .payline-2 { stroke: #7c4dff; animation-delay: 0.4s; }
  .payline-3 { stroke: #ffeb3b; animation-delay: 0.6s; }
  .payline-4 { stroke: #76ff03; animation-delay: 0.8s; }
  .payline-5 { stroke: #ff9100; animation-delay: 1s; }
  .payline-6 { stroke: #00e5ff; animation-delay: 1.2s; }
  .payline-7 { stroke: #ff1744; animation-delay: 1.4s; }
  .payline-8 { stroke: #64ffda; animation-delay: 1.6s; }
