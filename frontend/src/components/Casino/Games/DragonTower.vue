      ...
      <div class="game-header">
        <div class="stats-container">
          <div class="stat-box">
            ...
            <span class="stat-value">{{ dragonTowerStore.currentProfit.toFixed(2) }}€</span>
          </div>
          <div class="stat-box">
            <span class="stat-label">Multiplier</span>
            <span class="stat-value">{{ dragonTowerStore.currentMultiplier }}x</span>
          </div>
          <div class="stat-box">
...
>
  <div 
    v-for="(tile, tileIndex) in row.tiles.slice(0, DIFFICULTY_CONFIGS[dragonTowerStore.difficulty].columns)" 
    :key="tileIndex"
    class="tile"
    :class="{
      'revealed': tile.revealed,
      'dragon': tile.revealed && tile.isDragon,
      'dragon-clicked': tile.revealed && tile.isDragon && (9 - rowIndex) === dragonTowerStore.currentRow,
      'dragon-revealed': tile.revealed && tile.isDragon && (9 - rowIndex) !== dragonTowerStore.currentRow,
      'safe': tile.revealed && !tile.isDragon,
      'revealed-dragon': !tile.revealed && tile.isDragon && !dragonTowerStore.isGameActive
    }"
    @click="handleTileClick(9 - rowIndex, tileIndex)"
  >
    <span v-if="tile.revealed && !tile.isDragon" class="safe-emoji">💎</span>
    <span v-if="tile.revealed && tile.isDragon" class="dragon-emoji">🐉</span>
    <span v-if="!tile.revealed && tile.isDragon && !dragonTowerStore.isGameActive" class="revealed-dragon-emoji">��</span>
  </div>
</div>
      </div>
      ...
      <Transition name="modal">
        <div v-if="showWinModal" class="win-modal">
          <div class="modal-content">
            <h2>WIN!</h2>
            <div class="win-amount">{{ (dragonTowerStore.betAmount * dragonTowerStore.currentMultiplier).toFixed(2) }}€</div>
            <div class="multiplier">x{{ dragonTowerStore.currentMultiplier }}</div>
          </div>
        </div>
      </Transition>
    </div>
  ...
  import { useDragonTowerStore, DIFFICULTY_CONFIGS } from '../../../stores/casino/dragonTower';
  import { storeToRefs } from 'pinia';
  
  const dragonTowerStore = useDragonTowerStore();
  const { loading, isGameActive, currentMultiplier, currentProfit } = storeToRefs(dragonTowerStore);
...
const isCompletedRow = (rowIndex: number) => {
  ...
  watch(() => dragonTowerStore.difficulty, (newDifficulty) => {
    if (!dragonTowerStore.isGameActive) {
  ...
  .safe-emoji, .dragon-emoji, .revealed-dragon-emoji {
    position: absolute;
    top: 50%;
    left: 50%;
  ...
  .main-btn, .cashout-btn, .clear-btn {
    padding: 1rem;
    border-radius: 8px;
    border: none;
    font-size: 1.1rem;
